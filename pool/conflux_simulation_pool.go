...
var nMiningPool = 1                     // number of mining pools (the first nMiningPool nodes)
...
func receiveBlock(receiver int, blockId int) bool {
	...
	for !state.desc[c].IsEmpty() {
		...
		// if advChoice && max_desc != withholdedBlock {
		// 	releaseBlock = true
		// }
		c = max_desc
	}
...
func round(seq int, godSeed int, n int) {
	...
	for i := 0; i < blockNum; i++ {
		...
		r := int(rng.Int31n(int32(n + nBadNodes)))
		if r <= nBadNodes {
			addGenerateBlockEvent(ts, r%nMiningPool)
		} else {
			addGenerateBlockEvent(ts, r-nBadNodes)
		}
	}
